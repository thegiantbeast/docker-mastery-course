...
# get theme url (csrf token)
themeURL=$(curl -s 'http://localhost:8080/admin/appearance' -b $(dirname $0)/cookies.txt | sed -n 's/.*\(admin\/appearance\/default?theme=bootstrap&amp;token=[^"]*\).*/\1/p' | sed 's/&amp;/\&/')

# install and set theme
echo "Installing and setting bootstrap as the default theme"
curl -s -N -L "http://localhost:8080/$themeURL" -b $(dirname $0)/cookies.txt | grep -q '<em class="placeholder">Bootstrap</em> is now the default theme.'
